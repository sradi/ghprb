...
public class GhprbTrigger extends Trigger<AbstractProject<?, ?>> {
	...
	private static final Logger logger = Logger.getLogger(GhprbTrigger.class
			.getName());
	...
	public GhprbTrigger(String adminlist, String whitelist, String orgslist,
			String cron, String triggerPhrase, Boolean onlyTriggerPhrase,
			Boolean useGitHubHooks, Boolean permitAll,
			Boolean autoCloseFailedPullRequests,
			Boolean displayBuildErrorsOnDownstreamBuilds,
			String commentFilePath, List<GhprbBranch> whiteListTargetBranches,
			Boolean allowMembersOfWhitelistedOrgsAsAdmin, String msgSuccess,
			String msgFailure, String commitStatusContext)
			throws ANTLRException {
		this(adminlist, whitelist, orgslist, cron, triggerPhrase,
				onlyTriggerPhrase, useGitHubHooks, permitAll,
				autoCloseFailedPullRequests,
				displayBuildErrorsOnDownstreamBuilds, commentFilePath,
				whiteListTargetBranches, allowMembersOfWhitelistedOrgsAsAdmin,
				msgSuccess, msgFailure, commitStatusContext, "");
	}
	...
	public GhprbTrigger(String adminlist, String whitelist, String orgslist,
			...
			String commentFilePath, List<GhprbBranch> whiteListTargetBranches,
			Boolean allowMembersOfWhitelistedOrgsAsAdmin, String msgSuccess,
			String msgFailure, String commitStatusContext, String credentials)
			throws ANTLRException {
		super(cron);
	...
	public GhprbTrigger(String adminlist, String whitelist, String orgslist,
			...
			Boolean allowMembersOfWhitelistedOrgsAsAdmin, String msgSuccess,
			String msgFailure, String commitStatusContext,
			GhprbGithubCredentials credentials) throws ANTLRException {
		super(cron);
		this.adminlist = adminlist;
		this.whitelist = whitelist;
		this.orgslist = orgslist;
	...
	public void start(AbstractProject<?, ?> project, boolean newInstance) {
		...
		if (project.getProperty(GithubProjectProperty.class) == null) {
			logger.log(Level.INFO,
					"GitHub project not set up, cannot start ghprb trigger for job "
							+ this.project);
			return;
		}
		try {
	...
	Ghprb createGhprb(AbstractProject<?, ?> project) {
		return new Ghprb(project, this, getDescriptor().getPullRequests(
				project.getName()));
	}

	@Override
	public void stop() {
		logger.log(Level.INFO, "Stopping the ghprb trigger for project {0}",
				this.project);
		if (helper != null) {
			helper.stop();
			helper = null;
	...
	@Override
	public void run() {
		// triggers are always triggered on the cron, but we just no-op if we
		// are using GitHub hooks.
	...
	public QueueTaskFuture<?> startJob(GhprbCause cause, GhprbRepository repo) {
		...
		final String commitSha = cause.isMerged() ? "origin/pr/"
				+ cause.getPullID() + "/merge" : cause.getCommit();
		values.add(new StringParameterValue("sha1", commitSha));
		values.add(new StringParameterValue("ghprbActualCommit", cause
				.getCommit()));

		setTriggerSender(cause, values);
		setCommitAuthor(cause, values);

		final StringParameterValue pullIdPv = new StringParameterValue(
				"ghprbPullId", String.valueOf(cause.getPullID()));
		values.add(pullIdPv);
		values.add(new StringParameterValue("ghprbTargetBranch", String
				.valueOf(cause.getTargetBranch())));
		values.add(new StringParameterValue("ghprbSourceBranch", String
				.valueOf(cause.getSourceBranch())));
		values.add(new StringParameterValue("GIT_BRANCH", String.valueOf(cause
				.getSourceBranch())));
		// it's possible the GHUser doesn't have an associated email address
		values.add(new StringParameterValue("ghprbPullAuthorEmail", getString(
				cause.getAuthorEmail(), "")));
		values.add(new StringParameterValue("ghprbPullDescription",
				escapeQuotes(String.valueOf(cause.getShortDescription()))));
		values.add(new StringParameterValue("ghprbPullTitle", String
				.valueOf(cause.getTitle())));
		values.add(new StringParameterValue("ghprbPullLink", String
				.valueOf(cause.getUrl())));

		// add the previous pr BuildData as an action so that the correct change
		// log is generated by the GitSCM plugin
		// note that this will be removed from the Actions list after the job is
	...
	private void setTriggerSender(GhprbCause cause,
		...
		values.add(new StringParameterValue("ghprbTriggerAuthorEmail",
				triggerAuthorEmail));
	...
	private void setCommitAuthor(GhprbCause cause,
		...
		values.add(new StringParameterValue("ghprbActualCommitAuthor",
				authorName));
		values.add(new StringParameterValue("ghprbActualCommitAuthorEmail",
				authorEmail));
	}
	...
	/**
	 * Find the previous BuildData for the given pull request number; this may
	 * return null
	 */
	private BuildData findPreviousBuildForPullId(StringParameterValue pullIdPv) {
		// find the previous build for this particular pull request, it may not
		// be the last build
	...
	private ArrayList<ParameterValue> getDefaultParameters() {
		...
		ParametersDefinitionProperty pdp = this.job
	...
	public Boolean getAllowMembersOfWhitelistedOrgsAsAdmin() {
		return allowMembersOfWhitelistedOrgsAsAdmin != null
	...
	public Boolean isAutoCloseFailedPullRequests() {
		if (autoCloseFailedPullRequests == null) {
			Boolean autoClose = getDescriptor()
	...
	public Boolean isDisplayBuildErrorsOnDownstreamBuilds() {
		if (displayBuildErrorsOnDownstreamBuilds == null) {
			Boolean displayErrors = getDescriptor()
	...
	public GhprbBuilds getBuilds() {
		if (helper == null) {
			logger.log(
					Level.INFO,
					"The ghprb trigger for {0} wasn''t properly started - helper is null",
					this.project);
			return null;
		}
	...
	public static final class DescriptorImpl extends TriggerDescriptor {
		// GitHub username may only contain alphanumeric characters or dashes
		// and cannot begin with a dash
		private static final Pattern adminlistPattern = Pattern
				.compile("((\\p{Alnum}[\\p{Alnum}-]*)|\\s)*");

		/**
		 * These settings only really affect testing. When Jenkins calls
		 * configure() then the formdata will be used to replace all of these
		 * fields. Leaving them here is useful for testing, but must not be
		 * confused with a default. They also should not be used as the default
		 * value in the global.jelly file as this value is dynamic and will not
		 * be retained once configure() is called.
		 */
		...
		public DescriptorImpl() {
			...
			if (credentials.size() <= 0 && serverAPIUrl != null
					&& !serverAPIUrl.isEmpty()) {
				credentials.add(new GhprbGithubCredentials("default",
						serverAPIUrl, username, password, accessToken,
						publishedURL, false));
				save();
			}
		}
		...
		public boolean configure(StaplerRequest req, JSONObject formData)
			...
			requestForTestingPhrase = formData
					.getString("requestForTestingPhrase");
			...
			autoCloseFailedPullRequests = formData
					.getBoolean("autoCloseFailedPullRequests");
			displayBuildErrorsOnDownstreamBuilds = formData
					.getBoolean("displayBuildErrorsOnDownstreamBuilds");
			...
			if (formData.has("credential")) {
				...
				if (credential instanceof JSONArray) {
					...
					for (int i = 0; i < length; ++i) {
						credentials.add(new GhprbGithubCredentials(credsArray
								.getJSONObject(i)));
					}
				} else if (formData.get("credential") instanceof JSONObject) {
					credentials.add(new GhprbGithubCredentials(formData
							.getJSONObject("credential")));
				} else {
					logger.log(Level.INFO,
							"credential is not JSONArray and JSONObject");
				}
			} else if (formData.has("serverAPIUrl")) {
				logger.log(
						Level.INFO,
						"formData has serverAPIUrl("
								+ formData.getDouble("serverAPIUrl"));
				credentials.add(new GhprbGithubCredentials("default", formData
						.getString("serverAPIUrl"), formData
						.getString("username"), formData.getString("password"),
						formData.getString("accessToken"), formData
								.getString("publishedURL"), false));
			}
		...
		public FormValidation doCheckAdminlist(@QueryParameter String value)
			...
			if (!adminlistPattern.matcher(value).matches()) {
				return FormValidation
						.error("GitHub username may only contain alphanumeric characters or dashes and cannot begin with a dash. Separate them with whitespaces.");
			}
			return FormValidation.ok();
		}

		public FormValidation doCheckCredentialsServerAPIUrl(
			...
			return FormValidation
					.warning("GitHub API URI is \"https://api.github.com\". GitHub Enterprise API URL ends with \"/api/v3\"");
		}
		...
		public ConcurrentMap<Integer, GhprbPullRequest> getPullRequests(
				String projectName) {
			...
			if (jobs.containsKey(projectName)) {
				...
				jobs.put(projectName,
						(ConcurrentMap<Integer, GhprbPullRequest>) map);
		...
		public FormValidation doTestGithubAccess(
				@QueryParameter("credential.serverApiUrl") final String serverApiUrl,
				@QueryParameter("credential.accessToken") final String accessToken,
				@QueryParameter("credential.username") final String username,
			...
			try {
				...
				if (accessToken != null && !accessToken.isEmpty()) {
					...
					gh = GitHub.connectToEnterprise(serverApiUrl, username,
							password);
				}
				GHMyself me = gh.getMyself();
				return FormValidation.ok("Connected to " + serverApiUrl
						+ " as " + me.getName());
			} catch (IOException ex) {
				return FormValidation
						.error("GitHub API token couldn't be created: "
								+ ex.getMessage());
			}
		}
		...
		public FormValidation doCreateApiToken(
				...
				@QueryParameter("credential.password") final String password) {
			try {
				GitHub gh = GitHub.connectToEnterprise(serverApiUrl, username,
						password);
				GHAuthorization token = gh.createToken(Arrays.asList(
						GHAuthorization.REPO_STATUS, GHAuthorization.REPO),
						"Jenkins GitHub Pull Request Builder", null);
				return FormValidation.ok("Access token created: "
						+ token.getToken());
			} catch (IOException ex) {
				return FormValidation
						.error("GitHub API token couldn't be created: "
